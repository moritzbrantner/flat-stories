...
- Treat `2d-lab` as a 2D rendering experiment/evidence lab, not a Flat Stories runtime foundation. Do not import its `BenchmarkWorkload`, display list, or scene-shaped fixtures as application contracts. If Canvas becomes the measured bottleneck, evaluate Vello or another mature vector backend behind the existing Flat Stories render-frame adapter before implementing generic vector rasterization locally.
