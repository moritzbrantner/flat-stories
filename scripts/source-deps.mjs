...
const dependencies = [
  {
    ...
    smokeSpecifier: "@moenarch/editor-core",
    sourceEnv: "EDITOR_CORE_SOURCE",
    defaultSourceDir: path.resolve(rootDir, "../editor-core"),
    acceptedSourceNames: ["@moenarch/editor-core"],
  },
  {
    packageName: "@moritzbrantner/layer-editor",
    smokeSpecifier: "@moritzbrantner/layer-editor/core",
    sourceEnv: "LAYER_EDITOR_SOURCE",
    defaultSourceDir: path.resolve(rootDir, "../layer-editor"),
    acceptedSourceNames: ["@moritzbrantner/layer-editor"],
...
async function prepare() {
  ...
  for (const dependency of dependencies) {
    await prepareDependency(dependency);
  }
}

async function prepareDependency(dependency) {
  const sourceDir = resolveSourceDir(dependency);
  const sourceManifest = await readSourceManifest(dependency, sourceDir);
  run("bun", ["install", "--frozen-lockfile"], sourceDir);

  if (dependency.prepareSourceDependencies && sourceManifest.scripts?.["source:prepare"]) {
  ...
  await materializeSourcePackage(dependency, sourceDir, sourceManifest);
...
async function materializeSourcePackage(dependency, sourceDir, sourceManifest) {
  const targetDir = targetDirectory(dependency);
  await rm(targetDir, { force: true, recursive: true });
  await mkdir(targetDir, { recursive: true });
  await writeFile(path.join(targetDir, "package.json"), `${JSON.stringify(sourceManifest, null, 2)}\n`);

  for (const entry of sourceManifest.files ?? ["dist"]) {
